...
const CARD_COLORS=[
  {bg:"#fff5f3",border:"#ffd4cc",accent:"#ff2d00"},
  {bg:"#f0f4ff",border:"#c8d8ff",accent:"#3b82f6"},
  {bg:"#fff8ee",border:"#ffe0a8",accent:"#d97706"},
  {bg:"#eefbf4",border:"#b8e8d0",accent:"#16a34a"},
  {bg:"#f5f0ff",border:"#d4c4f8",accent:"#8b5cf6"},
  {bg:"#fef0f7",border:"#f5c0da",accent:"#db2777"},
  {bg:"#eefbff",border:"#b0e4f5",accent:"#0891b2"},
  {bg:"#fff4ee",border:"#ffd0b0",accent:"#ea580c"},
]
const COL_STYLES={
  todo:{label:"Total Stages",color:"#3b82f6",bg:"#f5f8ff",border:"#d8e4ff"},
  progress:{label:"In Progress",color:"#d97706",bg:"#fffbf0",border:"#ffe8b8"},
  completed:{label:"Completed",color:"#16a34a",bg:"#f0faf4",border:"#c0e8d0"},
}
...
function Btn({children,variant="primary",onClick,style={},disabled=false}){
  ...
  const v={primary:{background:"#ff2d00",color:"#fff",boxShadow:"0 2px 10px rgba(255,45,0,.2)"},secondary:{background:"#fff",color:"#6b6b80",border:"1px solid #e5e5ec"},ghost:{background:"transparent",color:"#6b6b80",padding:"8px 12px"},admin:{background:"linear-gradient(135deg,#ff2d00,#e02800)",color:"#fff",boxShadow:"0 2px 14px rgba(255,45,0,.25)"},mentor:{background:"linear-gradient(135deg,#3b82f6,#2563eb)",color:"#fff",boxShadow:"0 2px 14px rgba(59,130,246,.25)"}}
  ...
  return<div style={{marginBottom:18}}>{label&&<label style={{display:"block",fontSize:11,fontWeight:600,color:"#6b6b80",marginBottom:5,textTransform:"uppercase",letterSpacing:1.2}}>{label}</label>}
    <div style={{position:"relative"}}>{Icon&&<Icon size={15} style={{position:"absolute",left:13,top:"50%",transform:"translateY(-50%)",color:"#9898a8"}}/>}
      <input type={isP&&!show?"password":"text"} value={value} onChange={e=>onChange(e.target.value)} placeholder={placeholder} onKeyDown={onKeyDown} autoFocus={autoFocus} disabled={disabled}
        style={{width:"100%",padding:"13px 16px",paddingLeft:Icon?40:16,paddingRight:isP?40:16,background:disabled?"#f5f5f8":"#fff",border:"1px solid #e5e5ec",borderRadius:10,color:disabled?"#9898a8":"#1a1a2e",fontSize:13.5,fontFamily:"var(--font-body)",opacity:disabled?.7:1}}/>
...
function SearchDropdown({label,options,value,onChange,placeholder}){
  ...
  return<div ref={ref} style={{position:"relative"}}>
    ...
    <button onClick={()=>setOpen(!open)} style={{width:"100%",padding:"10px 13px",background:"#fff",border:"1px solid #e5e5ec",borderRadius:10,color:selected?"#1a1a2e":"#9898a8",fontSize:13,fontFamily:"var(--font-body)",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"space-between",gap:8,textAlign:"left"}}>
      ...
      <ChevronDown size={13} style={{flexShrink:0,transition:"transform .2s",transform:open?"rotate(180deg)":"none",opacity:.4,color:"#6b6b80"}}/></button>
...
function RolePickPage({onPick,onBack,mode}){const isLogin=mode==="login"
  ...
  <h2 style={{fontFamily:"var(--font-display)",fontSize:22,fontWeight:600,marginBottom:4,color:"#1a1a2e"}}>{isLogin?"Login":"Create Account"}</h2><p style={{color:"#6b6b80",fontSize:13,marginBottom:28,fontWeight:400}}>Select your role</p>
  <div style={{display:"flex",flexDirection:"column",gap:10}}>
    ...
    <button onClick={()=>onPick("mentor")} style={{padding:"16px 20px",borderRadius:12,border:"1px solid #c8d8ff",background:"#f5f8ff",color:"#1a1a2e",fontSize:14,fontWeight:600,cursor:"pointer",fontFamily:"var(--font-body)",display:"flex",alignItems:"center",gap:12,transition:"all .2s"}} onMouseEnter={e=>e.currentTarget.style.background="#eef4ff"} onMouseLeave={e=>e.currentTarget.style.background="#f5f8ff"}>
      ...
      <div style={{textAlign:"left"}}><div style={{fontWeight:600}}>Mentor</div><div style={{fontSize:11.5,color:"#6b6b80",fontWeight:400,marginTop:1}}>Manage assigned tasks</div></div></button>
  </div>
...
function LoginPage({onSubmit,onBack,loading,prefillEmail,role}){const[email,setEmail]=useState(prefillEmail||"");const[pw,setPw]=useState("");const[err,setErr]=useState("")
  ...
  return<div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:"#f8f8fa"}}><div className="fade-up" style={{background:"#fff",border:"1px solid #e5e5ec",borderRadius:20,padding:"44px 36px",width:"100%",maxWidth:420,boxShadow:"0 8px 32px rgba(0,0,0,.06)"}}>
    ...
    <h2 style={{fontFamily:"var(--font-display)",fontSize:22,fontWeight:600,textAlign:"center",marginBottom:4,color:"#1a1a2e"}}>{isA?"Admin":"Mentor"} Login</h2><p style={{textAlign:"center",color:"#6b6b80",fontSize:13,marginBottom:28,fontWeight:400}}>Enter your credentials</p>
...
function CreateEmailPage({onSubmit,onBack,loading,role}){const[email,setEmail]=useState("");const isA=role==="admin"
  return<div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:"#f8f8fa"}}><div className="fade-up" style={{background:"#fff",border:"1px solid #e5e5ec",borderRadius:20,padding:"44px 36px",width:"100%",maxWidth:420,boxShadow:"0 8px 32px rgba(0,0,0,.06)"}}>
    ...
    <h2 style={{fontFamily:"var(--font-display)",fontSize:22,fontWeight:600,textAlign:"center",marginBottom:4,color:"#1a1a2e"}}>{isA?"Admin":"Mentor"} Account</h2><p style={{textAlign:"center",color:"#6b6b80",fontSize:13,marginBottom:28,fontWeight:400}}>Enter your registered email</p>
...
function OTPPage({email,onVerify,onBack,loading}){const[otp,setOtp]=useState(["","","","","",""]);const refs=useRef([])
  ...
  return<div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:"#f8f8fa"}}><div className="fade-up" style={{background:"#fff",border:"1px solid #e5e5ec",borderRadius:20,padding:"44px 36px",width:"100%",maxWidth:420,textAlign:"center",boxShadow:"0 8px 32px rgba(0,0,0,.06)"}}>
    <div style={{width:50,height:50,borderRadius:14,background:"linear-gradient(135deg,#d97706,#b45309)",display:"flex",alignItems:"center",justifyContent:"center",margin:"0 auto 18px"}}><Mail size={22} color="#fff"/></div>
    <h2 style={{fontFamily:"var(--font-display)",fontSize:22,fontWeight:600,marginBottom:4,color:"#1a1a2e"}}>Verify OTP</h2><p style={{color:"#6b6b80",fontSize:13,fontWeight:400}}>Code sent to</p><p style={{color:"#ff2d00",fontSize:12.5,fontWeight:600,marginBottom:26}}>{email}</p>
...
function CreatePasswordPage({email,onSubmit,loading}){const[pw,setPw]=useState("");const[cf,setCf]=useState("");const[err,setErr]=useState("")
  ...
  return<div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:"#f8f8fa"}}><div className="fade-up" style={{background:"#fff",border:"1px solid #e5e5ec",borderRadius:20,padding:"44px 36px",width:"100%",maxWidth:420,boxShadow:"0 8px 32px rgba(0,0,0,.06)"}}>
    ...
    <div style={{padding:"8px 12px",borderRadius:8,background:"#f0f4ff",border:"1px solid #d8e4ff",color:"#3b82f6",fontSize:11,marginBottom:18,lineHeight:1.5,fontWeight:400}}>Min 6 chars, 1 uppercase, 1 number</div>
    {err&&<div style={{padding:"9px 13px",borderRadius:8,background:"#fef5f5",border:"1px solid #f5c0c0",color:"#dc2626",fontSize:12.5,marginBottom:14,display:"flex",alignItems:"center",gap:7}}><AlertCircle size={13}/>{err}</div>}
    ...
    <Btn onClick={go} disabled={loading} style={{width:"100%",justifyContent:"center",padding:14,borderRadius:12,background:"linear-gradient(135deg,#16a34a,#15803d)",boxShadow:"0 2px 10px rgba(22,163,74,.2)"}}>{loading?"Creating...":<><CheckCircle2 size={15}/> Create Password</>}</Btn>
  </div></div>}
...
function TaskCard({task,index,onClick}){const c=CARD_COLORS[index%CARD_COLORS.length];const stages=task.stages||[];const done=stages.filter(s=>s.status==="completed").length;const prog=stages.filter(s=>s.status==="progress").length
  return<div onClick={onClick} style={{background:c.bg,border:`1px solid ${c.border}`,borderRadius:14,padding:"18px 20px",cursor:"pointer",transition:"all .2s"}}
    ...
    <div style={{marginTop:10,display:"flex",alignItems:"center",gap:7}}>
      {task.responsible?<><Avatar name={task.responsible.name} size={20}/><span style={{fontSize:11.5,color:"#6b6b80",fontWeight:400}}>{task.responsible.name}</span></>
...
function StageCard({stage,idx,colKey,onDragStart,canDrag,onEdit,onDelete,canEdit,mentors,teamMembers,onAssign}){
  ...
  return<div draggable={canDrag} onDragStart={e=>{if(!canDrag){e.preventDefault();return}onDragStart(e,idx,colKey)}}
    style={{background:"#fff",border:"1px solid #e5e5ec",borderRadius:10,padding:"12px 14px",cursor:canDrag?"grab":"default",transition:"all .15s",boxShadow:"0 1px 3px rgba(0,0,0,.04)"}}
    onMouseEnter={e=>{if(canDrag)e.currentTarget.style.boxShadow="0 3px 10px rgba(0,0,0,.08)"}} onMouseLeave={e=>{e.currentTarget.style.boxShadow="0 1px 3px rgba(0,0,0,.04)"}}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:7}}>
      <p style={{fontSize:13,fontWeight:500,color:"#1a1a2e",lineHeight:1.4,flex:1,minWidth:0}}>{stage.title}</p>
...
function KanbanBoard({stages,onSaveStages,canManage,isTM,currentUser,mentors,teamMembers,addToast}){
      ...
      <div style={{padding:10,display:"flex",flexDirection:"column",gap:7,flex:1,overflowY:"auto"}}>
        {items.map((stage,idx)=>{const canDrag=canManage||(stage.assignedTo===currentUser?.email)
          if(editIdx===idx&&editCol===colKey)return<div key={idx} style={{background:"#fff",border:"1px solid #d0d0da",borderRadius:10,padding:"10px 12px"}}>
            ...
            <div style={{display:"flex",gap:5}}><button onClick={saveEd} style={{background:"#f0faf4",border:"1px solid #c0e8d0",borderRadius:7,padding:"5px 10px",color:"#16a34a",fontSize:11.5,fontWeight:600,cursor:"pointer",display:"flex",alignItems:"center",gap:3}}><Check size={11}/>Save</button>
  ...
  return<div>
    {(canManage||isTM)&&<div style={{display:"flex",gap:8,marginBottom:14}}>
      <input value={newStage} onChange={e=>setNewStage(e.target.value)} onKeyDown={e=>e.key==="Enter"&&addNew()} placeholder="Add new stage..." style={{flex:1,padding:"9px 13px",background:"#fff",border:"1px solid #e5e5ec",borderRadius:9,color:"#1a1a2e",fontSize:12.5,fontFamily:"var(--font-body)"}}/>
...
function AdminDashboard({user,tasks,mentors,onLogout,onRefresh,addToast}){const[search,setSearch]=useState("");const[openTask,setOpenTask]=useState(null)
  ...
  return<div style={{minHeight:"100vh",background:"#f8f8fa"}}><div style={{padding:"14px 26px",borderBottom:"1px solid #e5e5ec",display:"flex",justifyContent:"space-between",alignItems:"center",background:"#fff"}}>
    <div style={{display:"flex",alignItems:"center",gap:10}}><div style={{width:34,height:34,borderRadius:9,background:"linear-gradient(135deg,#ff2d00,#cc2400)",display:"flex",alignItems:"center",justifyContent:"center"}}><ClipboardList size={16} color="#fff"/></div>
      <div><h1 style={{fontFamily:"var(--font-display)",fontSize:15,fontWeight:600,color:"#1a1a2e"}}>ProjectSpace</h1><p style={{fontSize:10.5,color:"#9898a8",fontWeight:400}}>Admin Dashboard</p></div></div>
    <div style={{display:"flex",alignItems:"center",gap:12}}><div style={{padding:"5px 12px",borderRadius:7,background:"#fff5f3",border:"1px solid #ffd4cc",display:"flex",alignItems:"center",gap:5}}><Shield size={12} color="#ff2d00"/><span style={{fontSize:11,fontWeight:600,color:"#ff2d00"}}>Admin</span></div>
      <Avatar name={user.name} size={28}/><span style={{fontSize:12.5,color:"#6b6b80",fontWeight:500}}>{user.name}</span><Btn variant="ghost" onClick={onLogout} style={{color:"#9898a8"}}><LogOut size={14}/></Btn></div></div>
    <div style={{padding:"22px 26px",maxWidth:1140,margin:"0 auto"}}>
      ...
      <div style={{marginBottom:22,position:"relative"}}><Search size={15} style={{position:"absolute",left:13,top:"50%",transform:"translateY(-50%)",color:"#9898a8"}}/>
        <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search tasks, mentors, stages..." style={{width:"100%",padding:"11px 16px 11px 40px",background:"#fff",border:"1px solid #e5e5ec",borderRadius:10,color:"#1a1a2e",fontSize:13,fontFamily:"var(--font-body)",boxShadow:"0 1px 3px rgba(0,0,0,.03)"}}/></div>
...
function MentorDashboard({user,tasks,mentors,onLogout,onRefresh,addToast}){const[search,setSearch]=useState("");const[openTask,setOpenTask]=useState(null)
  ...
  return<div style={{minHeight:"100vh",background:"#f8f8fa"}}><div style={{padding:"14px 26px",borderBottom:"1px solid #e5e5ec",display:"flex",justifyContent:"space-between",alignItems:"center",background:"#fff"}}>
    <div style={{display:"flex",alignItems:"center",gap:10}}><div style={{width:34,height:34,borderRadius:9,background:"linear-gradient(135deg,#3b82f6,#2563eb)",display:"flex",alignItems:"center",justifyContent:"center"}}><ClipboardList size={16} color="#fff"/></div>
      <div><h1 style={{fontFamily:"var(--font-display)",fontSize:15,fontWeight:600,color:"#1a1a2e"}}>ProjectSpace</h1><p style={{fontSize:10.5,color:"#9898a8",fontWeight:400}}>Mentor Portal</p></div></div>
    <div style={{display:"flex",alignItems:"center",gap:12}}><div style={{padding:"5px 12px",borderRadius:7,background:"#f0f4ff",border:"1px solid #c8d8ff",display:"flex",alignItems:"center",gap:5}}><User size={12} color="#3b82f6"/><span style={{fontSize:11,fontWeight:600,color:"#3b82f6"}}>Mentor</span></div>
      <Avatar name={user.name} size={28}/><span style={{fontSize:12.5,color:"#6b6b80",fontWeight:500}}>{user.name}</span><Btn variant="ghost" onClick={onLogout} style={{color:"#9898a8"}}><LogOut size={14}/></Btn></div></div>
    <div style={{padding:"22px 26px",maxWidth:1140,margin:"0 auto"}}>
      <div style={{marginBottom:22,padding:"18px 22px",background:"#fff",border:"1px solid #e5e5ec",borderRadius:14,boxShadow:"0 1px 4px rgba(0,0,0,.03)"}}><h2 style={{fontFamily:"var(--font-display)",fontSize:20,fontWeight:600,marginBottom:3,color:"#1a1a2e"}}>Welcome, {user.name}</h2><p style={{color:"#9898a8",fontSize:13,fontWeight:400}}>{user.email}</p></div>
